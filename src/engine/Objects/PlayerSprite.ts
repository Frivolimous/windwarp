...
export class PlayerSprite extends PIXI.Container {
  ...
  drawPlayer() {
    ...
    this.view.stroke({ width: 2, color: 0 });
    this.leftHand.circle(0, 0, this.collider.width * 0.3).fill(0xff6633).stroke({ width: 2, color: 0 });
    this.rightHand.circle(0, 0, this.collider.width * 0.3).fill(0xff6633).stroke({ width: 2, color: 0 });
  }
  ...
  updateHands() {
    ...
    switch(this.movementState) {
      case 'idle': 
        this.setChildIndex(this.rightHand, 2);
        this.setChildIndex(this.leftHand, 2);
        ...
        break;
      case 'walking':
        this.setChildIndex(this.rightHand, 0);
        this.setChildIndex(this.leftHand, 0);
        if (this.vX < 0) {
          let amt = Math.max(this.vX, -2);
          dly = this.collider.height * (0.6 - amt * 0.01);
          dry = this.collider.height * (0.6 - amt * 0.02);
          dlx = this.collider.width * (-0.1 - amt * 0.1);
          drx = this.collider.width * (1.1 - amt * 0.5);
        } else {
          let amt = Math.min(this.vX, 2);
          dly = this.collider.height * (0.6 + amt * 0.02);
          dry = this.collider.height * (0.6 + amt * 0.01);
          dlx = this.collider.width * (-0.1 - amt * 0.5);
          drx = this.collider.width * (1.1 - amt * 0.1);
        }
        break;
    ...
    this.leftHand.x += (dlx - this.leftHand.position.x) * this.animationSpeed;
    this.leftHand.y += (dly - this.leftHand.position.y) * this.animationSpeed;
    this.rightHand.x += (drx - this.rightHand.position.x) * this.animationSpeed;
    this.rightHand.y += (dry - this.rightHand.position.y) * this.animationSpeed;
  }
}
